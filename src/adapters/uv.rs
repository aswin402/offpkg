...
impl UvAdapter {
    ...
    pub fn add(&mut self, pkg: &str, _skip_config: bool) -> Result<()> {
        ...
        if !Path::new(&cached.cache_path).exists() {
            drop(sp);
            return Err(anyhow!("Cache file missing. Re-run: offpkg uv install {}", pkg));
        }

        if !self.cache.verify_checksum(Path::new(&cached.cache_path), &cached.checksum)? {
            drop(sp);
            return Err(anyhow!("Checksum mismatch. Re-run: offpkg uv install {}", pkg));
        }
        sp.finish(
            Label::Resolve,
        ...
        let output = Command::new("uv")
            .args([
                "add", "--frozen", "--no-index",
                "--find-links", &cache_dir.to_string_lossy(),
                pkg,
            ])
            .output()
        ...
        self.tui.print_line(Label::Link, &format!("{} → .venv", cached.name), Some(&format!("v{}", cached.version)));
        self.tui.print_line(Label::Done, "1 package installed", Some("no network used"));
    ...
    pub async fn install(&mut self, pkg: &str) -> Result<()> {
        ...
        sp.finish(Label::Resolve, &format!("{}@{}", pkg, version), Some("PyPI"));
        ...
        let doc_content = fetch_docs("uv", pkg, &version).await
            .unwrap_or_else(|_| format!("# {pkg} v{version}\n\nNo docs available.\n"));
        self.docs.save_docs("uv", pkg, &doc_content)?;
        sp.finish(Label::Info, &format!("{} docs cached", pkg), Some("~/.offpkg/docs/uv/"));
    ...
    pub async fn install_all(&mut self) -> Result<()> {
        ...
        if !pyproject_path.exists() {
            return Err(anyhow!("No pyproject.toml found. Run from inside your Python project."));
        }
        ...
        let pkg_names: Vec<String> = deps
            ...
            .map(|d| {
                d.split(|c: char| !c.is_alphanumeric() && c != '-' && c != '_')
                    .next().unwrap_or(d).to_lowercase()
            })
            .collect();

        self.tui.print_line(Label::Info, &format!("found {} dependencies in pyproject.toml", pkg_names.len()), None);

        let mut success = 0;
        let mut failed: Vec<String> = vec![];

        for pkg in &pkg_names {
            let already = self.db.list_packages(Some("uv"))?.into_iter().any(|p| p.name == *pkg);
            if already {
